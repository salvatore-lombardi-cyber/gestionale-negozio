...
/**
 * Model per Tipo di Pagamento con OWASP Security Best Practices 2025
 * Gestione tipi di pagamento (contanti, bonifico, assegno, carta credito, etc.)
 */
class PaymentType extends Model
{
    ...
    {
        return [
            'code' => 'required|string|max:20|unique:payment_types,code|regex:/^[A-Z0-9_-]+$/',
    ...
    public static function validationRulesForUpdate(int $id): array
    {
        ...
        $rules['code'] = 'required|string|max:20|unique:payment_types,code,' . $id . '|regex:/^[A-Z0-9_-]+$/';
        return $rules;
    }
    ...
    protected static function booted()
    {
        ...
        static::deleting(function ($model) {
            \Log::warning('Deleting PaymentType', [
                'id' => $model->id,
                'code' => $model->code,
                'name' => $model->name,
                'user_id' => auth()->id(),
