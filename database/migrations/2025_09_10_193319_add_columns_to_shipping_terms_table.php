...
return new class extends Migration
{
    ...
    {
        Schema::table('shipping_terms', function (Blueprint $table) {
            $table->string('incoterm_code', 10)->nullable()->after('description');
            $table->enum('type', ['factory', 'delivery', 'mixed'])->default('mixed')->after('incoterm_code');
            $table->boolean('customer_pays_shipping')->default(false)->after('type');
        });

        // Inserimento dati di esempio dopo aver aggiunto le colonne
    ...
    {
        Schema::table('shipping_terms', function (Blueprint $table) {
            $table->dropColumn(['incoterm_code', 'type', 'customer_pays_shipping']);
        });
    }
};
