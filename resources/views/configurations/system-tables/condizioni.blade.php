...
function validateCode() {
    ...
    if (code.length >= 2) {
        fetch(`{{ route('configurations.system-tables.show', 'conditions') }}?check_duplicate=${encodeURIComponent(code)}`)
...
function editItem(id) {
    ...
    fetch(`{{ route('configurations.system-tables.show', 'conditions') }}/${id}/edit`)
        .then(response => response.json())
        .then(data => {
...
function deleteItem(id, name) {
    if (confirm(`Sei sicuro di voler eliminare la condizione "${name}"?\n\nQuesta azione non può essere annullata.`)) {
        ...
        fetch(`{{ route('configurations.system-tables.show', 'conditions') }}/${id}`, {
            method: 'POST',
            body: formData,
            headers: {
...
function saveCondition() {
    ...
    let url = '{{ route("configurations.system-tables.store", "conditions") }}';
    let method = 'POST';
    
    if (editingId) {
        url = `{{ route('configurations.system-tables.show', 'conditions') }}/${editingId}`;
...
function exportData() {
    window.location.href = `{{ route('configurations.system-tables.export', 'conditions') }}`;
}
